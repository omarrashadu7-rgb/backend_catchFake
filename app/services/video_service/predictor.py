...
async def predict_video(file_path: str):
    ...
    async with httpx.AsyncClient(timeout=600) as client:
        ...
        if not isinstance(upload_data, list) or not upload_data:
            raise AIServiceUnavailable("Unexpected upload response")
    ...
    if not data_lines:
        raise AIServiceUnavailable("No data in Gradio response")
    ...
    if not isinstance(result_data, (list, tuple)) or len(result_data) < 1:
        raise AIServiceUnavailable("Unexpected model response format")
